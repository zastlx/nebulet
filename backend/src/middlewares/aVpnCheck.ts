...
let template = (header:String, body:String, pageTitle:String) => `
<head>
    <style>
...
export default async (req: Request, res: Response, next: NextFunction) => {
    if (config.vpnCheck.bypasses.some(allowedIp => req.ip.startsWith(allowedIp))) return next();
    if (whitelist.includes(req.ip)) return next();
    if (blacklist.includes(req.ip)) return res.send(template('Not Allowed', 'You are using a VPN/Proxy and hence cannot access Nebulet.', 'IP Denied'));

    if (req.path.includes('api/fingerprint')) {
        axios.get(`https://vpnapi.io/api/${req.ip}?key=${config.vpnCheck.apiKey}`).then((r) => {
    ...
    } else if (req.path.startsWith('/fingerprint')) res.send(template('Loading...', 'We are working internally to check your connection.', 'Fingerprint'));
    else res.redirect('/fingerprint');
};