    ...
    post: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            if (typeof username !== "string" || typeof password !== "string")
                return res.send(403).send("Username and password must be strings.");
            if (/[^-\]_.a-zA-Z0-9]/.test(username))
                return res.status(403).send("Username cannot contain special charaters.");
            ...
            if (result.length < 1)
                return res.status(403).send("There is no account with that username.");
